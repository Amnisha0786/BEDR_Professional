...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      'fixed inset-0 z-50 border-0 bg-black/80 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0',
      className,
    )}
    {...props}
  />
...
const DialogContent = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof DialogPrimitive.Content> & TDialogProps
>(
  (
    {
      className,
      ...
      >
        ...
        {!hidecross && (
          <DialogPrimitive.Close className='absolute right-4 top-4 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:outline-none disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground'>
            {oulinedCross ? (
              <CircleX className='h-[18px] w-[18px] text-gray' />
            ) : (
              <X className='h-4 w-4' />
            )}
...
const DialogFooter = ({
  ...
  <div
    className={cn(
      'flex flex-col-reverse gap-[10px] sm:flex-row sm:justify-end sm:space-x-2',
      className,
    )}
    {...props}
  />
